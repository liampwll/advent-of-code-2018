...
pub fn main() !void {
    ...
    std.debug.warn("{}\n", try cpu(&direct_allocator.allocator, @embedFile("input/19")));
}

fn cpu(allocator: *std.mem.Allocator, input: []const u8) !u32 {
    ...
    // var state = State{.regs = []usize{1, 0, 0, 0, 0, 0}};

    var i: usize = 1;
    while (i < 100) : (i += 1) {
        var state = State{.regs = []usize{0, 1, 1, 4, i, 1}};

        while (state.regs[pc_reg] < program.count()) : (state.regs[pc_reg] += 1) {
            const inst = program.at(state.regs[pc_reg]);
            inst.op(&state, inst.a, inst.b, inst.c);
        }
        std.debug.warn("{} {}\n", i, state.regs[0]);
    }
    // result is sum of factors

    var res: u32 = 0;
    var j: u32 = 1;
    while (j <= 10551378) : (j += 1) {
        if (10551378 % j == 0) {
            res += j;
        }
    }
