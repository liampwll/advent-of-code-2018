...
fn battle(allocator: *std.mem.Allocator, input: []const u8) u32 {
    var goblins: u32 = 0;
    var elves: u32 = 0;
    var elf_attack: u32 = 0;
    var turn: u32 = 1;
    var grid: [GRID_SIZE][GRID_SIZE]Tile = undefined;

    while (elves != 10) : (elf_attack += 1) {
        elves = 0;
        goblins = 0;
        turn = 1;
                ...
                for (line) |*tile, x| {
                    if (tile.unit != null and tile.unit.?.last_turn != turn) {
                        ...
                        if (select_target(&grid, p, unit.kind)) |t| {
                            ...
                            if (t_unit.health <= if (unit.kind == UnitKind.Elf) elf_attack else ATTACK) {
                                switch (t_unit.kind) {
                                ...
                                t_unit.health -= if (unit.kind == UnitKind.Elf) elf_attack else ATTACK;
                            }
                        }
                    }
                }
