...
fn cpu(allocator: *std.mem.Allocator, input: []const u8) !void {
    ...
    var state = State{.regs = []usize{0, 0, 0, 0, 0, 0}};
    while (true) : (state.regs[pc_reg] += 1) {
        const inst = program.at(state.regs[pc_reg]);
        inst.op(&state, inst.a, inst.b, inst.c);

        if (state.regs[pc_reg] == 28) {
            if (!found.contains(state.regs[1])) {
                _ = try found.put(state.regs[1], {});
                std.debug.warn("{}\n", state.regs[1]);
            }
            state.regs[2] = 0;
        }
...
fn arrayEql_b(key_a: [6]usize, key_b: [6]usize) bool {
    for (key_a) |x, i| {
        if (key_b[i] != x) {
            return false;
        }
...
fn fnv1a32_b(key: [6]usize) u32 {
    var res: u32 = 2166136261;
    for (@sliceToBytes(key[0..])) |char| {
        res = (res ^ @intCast(u32, char)) *% 16777619;
    }
    return res;
}
